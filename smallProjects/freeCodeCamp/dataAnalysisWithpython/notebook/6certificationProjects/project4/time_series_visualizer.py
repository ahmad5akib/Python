...
def draw_line_plot():
    ...
    sns.lineplot(x="date", y="value", ax=ax, data=df).set(
...
def draw_bar_plot():
    ...
    table = pd.pivot_table(
        df_bar, values="value", index="Years", columns="Months", dropna=False
    )
    months = [
        "January",
        "February",
    ...
    # Draw bar plot
    ax = table.plot(
        kind="bar", figsize=(16, 9), xlabel="Years", ylabel="Average Page Views"
    )
    bar = ax.get_figure()
...
def draw_box_plot():
    ...
    # Yearly boxplot
    sns.boxplot(data=df_box, x="year", y="value", ax=ax1).set(
